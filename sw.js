const CACHE_NAME = 'lich-van-trung-pwa-v20'; // v20: chặn cache file HTML lỗi, redirect lucide.min.js → CDN
...
const ASSETS_TO_CACHE = [
  ...
  // lucide-react dùng CDN, không cache local
...
// RỦI RO 2: Bọc importScripts trong khối try...catch để tránh crash Service Worker nếu mạng chập chờn hoặc offline lúc cập nhật
try {
  importScripts('https://www.gstatic.com/firebasejs/9.23.0/firebase-app-compat.js');
  importScripts('https://www.gstatic.com/firebasejs/9.23.0/firebase-firestore-compat.js');
} catch (e) {
  console.error('[SW] Không thể tải thư viện Firebase qua importScripts. Đang chạy chế độ dự phòng ngoại tuyến.', e);
}
...
// RỦI RO 1: Kiểm tra xem Firebase đã được tải và chưa khởi tạo ứng dụng nào trước khi setup, tránh lỗi khởi tạo lại khi SW restart
let db = null;
if (typeof firebase !== 'undefined') {
  ...
  console.warn('[SW] Firebase không sẵn sàng. Tiến trình đồng bộ ngầm và lấy dữ liệu online tạm thời bị vô hiệu hóa.');
}
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      ...
      return Promise.all([
        self.clients.claim(),
        checkAndShowNotifications().catch(() => {})
      ]);
    })
  );
});
...
self.addEventListener('fetch', (event) => {
  ...
  // Bỏ qua hoàn toàn request đến lucide.min.js local (file lỗi) — dùng CDN thay thế
  if (requestUrl.includes('lucide.min.js')) {
    event.respondWith(fetch('https://esm.sh/lucide-react@0.383.0?external=react&bundle'));
    return;
  }
  ...
  if (event.request.mode === 'navigate') {
      event.waitUntil(checkAndShowNotifications().catch(() => {}));
  }
  ...
  event.respondWith(
    caches.match(event.request).then((cachedResponse) => {
      ...
      return fetch(event.request).then((networkResponse) => {
        ...
        // Chỉ cache response hợp lệ và đúng loại (tránh cache trang HTML lỗi 404 vào slot JS/CSS)
        const isBadHtml = contentType.includes('text/html') &&
          !requestUrl.endsWith('.html') && !requestUrl.endsWith('/');

        if ((status === 200 || status === 0) && !isBadHtml) {
...
// RỦI RO 3: Chuyển sang sử dụng async/await hoàn toàn và chuyển tiếp lỗi (propagate error) lên waitUntil
self.addEventListener('sync', (event) => {
    if (event.tag === 'sync-data') {
        event.waitUntil(
            (async () => {
                // Đảm bảo nếu syncPendingData() ném ra lỗi, event.waitUntil sẽ bắt được để lên lịch thử lại (retry)
...
async function syncPendingData() {
    try {
        // Đảm bảo Firebase SDK đã tải và hoạt động thành công
        if (!db) {
            throw new Error('Firebase Firestore SDK chưa được nạp đệm hoặc chưa khởi tạo thành công.');
        }
        ...
        for (const record of records) {
            try {
                ...
                // Xóa bản ghi ra khỏi hàng đợi chỉ khi nạp thành công lên Firebase Firestore
        ...
        // RỦI RO 3: Nếu có ít nhất một bản ghi đồng bộ lỗi, ta ném lỗi ra ngoài để trình duyệt không xóa hàng đợi và thử lại sau
        if (hasFailed) {
            throw firstError || new Error("Quá trình đồng bộ ngầm một số bản ghi bị thất bại.");
        }
    } catch (e) {
        console.error('[Sync] Lỗi tiến trình đồng bộ ngầm:', e);
        // Ném lỗi ngược lên sự kiện sync để trigger cơ chế retry tự động của trình duyệt
        throw e;
    }
}